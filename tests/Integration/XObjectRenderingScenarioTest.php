...
final class XObjectRenderingScenarioTest extends TestCase
{
    ...
    public function testReusableXObjectScenarios(string $html, float $width, float $height, int $maxStreamLength): void
    {
        $compiler = new XObjectTemplateCompiler();
        $result = $compiler->compile(new CompileRequest(html: $html, width: $width, height: $height));
    ...
    /**
     * @return iterable<string, array{html: string, width: float, height: float, maxStreamLength: int}>
     */
